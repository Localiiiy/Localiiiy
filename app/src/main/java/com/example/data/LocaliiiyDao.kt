...
interface LocaliiiyDao {
    ...
    @Query("SELECT * FROM posts WHERE isFollowing = 1 ORDER BY timestamp DESC")
    ...
    @Query("SELECT * FROM clips WHERE distanceKm <= :maxKm ORDER BY distanceKm ASC, timestamp DESC")
    fun getNearbyClips(maxKm: Double): Flow<List<ClipEntity>>

    @Query("SELECT * FROM clips WHERE username = :username ORDER BY timestamp DESC")
    ...
    @Query("UPDATE clips SET isLiked = :isLiked, likesCount = likesCount + :delta WHERE id = :id")
    suspend fun updateClipLike(id: Long, isLiked: Boolean, delta: Int)

    @Query("UPDATE clips SET isSaved = :isSaved WHERE id = :id")
    suspend fun updateClipSaved(id: Long, isSaved: Boolean)

    @Query("UPDATE clips SET isFollowing = :isFollowing WHERE id = :id")
    suspend fun updateClipFollowing(id: Long, isFollowing: Boolean)

    suspend fun updateClipConnected(id: Long, isConnected: Boolean) = updateClipFollowing(id, isConnected)

    @Query("UPDATE clips SET caption = :caption, location = :location, landmark = :landmark WHERE id = :id")
    ...
    suspend fun updateOtherUserConnected(username: String, isConnected: Boolean, delta: Int) =
        updateOtherUserFollowing(username, isConnected, delta)
    ...
    suspend fun updateStudioCreatorConnected(creatorUsername: String, isConnected: Boolean) =
        updateStudioCreatorSubscribed(creatorUsername, isConnected)
    ...
    // --- Cyberstalking Immutable Records (Universal Anti-Stalking Protocol) ---
    @Query("SELECT * FROM cyberstalking_incident_records ORDER BY timestamp DESC")
    fun getAllCyberstalkingIncidents(): Flow<List<CyberstalkingIncidentEntity>>

    @Query("SELECT * FROM cyberstalking_incident_records WHERE id = :id")
    suspend fun getCyberstalkingIncidentById(id: String): CyberstalkingIncidentEntity?

    @Query("SELECT COUNT(*) FROM cyberstalking_incident_records")
    fun getCyberstalkingIncidentCount(): Flow<Int>
